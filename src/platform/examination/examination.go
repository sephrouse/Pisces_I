...
func storeAllWords() error {
	fh, err := os.Open("./public/assets/vocabulary/arrangedVocabulary.txt")
	if err != nil {
...
func GenerateEnToCnExamination() Examination {
	...
	for i := 0; i < cQuestionNum; i++ {
		...
		e.Questions[i].Subject = fmt.Sprintf("%d.  %s", i+1, words[no].enWord)
...
func GenerateCnToEnExamination() Examination {
	...
	for i := 0; i < cQuestionNum; i++ {
		...
		e.Questions[i].Subject = fmt.Sprintf("%d.  %s", i+1, words[no].cnWord)
...
func init() {
	...
	if nil != storeAllWords() {
		fmt.Println("package examination init error.")
		return
	}
}
